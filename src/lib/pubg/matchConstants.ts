...
// 목록 카드용 요약 캐시(실측 291바이트). 종료된 매치는 불변이라 길게 잡을 수 있다.
//
// 그래도 7일로 끊는다. 여기서 TTL은 신선도가 아니라 **용량**을 정한다 — 매치는 안 변하지만
// 매일 새 사람·새 매치가 조회되며 키가 쌓이고, TTL이 만료돼야 빠진다.
// 하루 신규 2만 건이면 30일은 175MB(Upstash 무료 256MB의 68%), 7일은 41MB다.
//
// 만료된 매치를 다시 열어도 한도는 안 쓴다(아래 MATCH_DETAIL_TTL 주석 참고).
export const MATCH_SUMMARY_TTL = 60 * 60 * 24 * 7;
...
export const TELEMETRY_SCHEMA_VERSION = "v10";
